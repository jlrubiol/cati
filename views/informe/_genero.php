...
$subnum_tabla = 0;
foreach($generos as $centro_id => $datos_del_centro) {
    $centro = Centro::findOne($centro_id);
    $anyos = array_shift($datos_del_centro);
    $subnum_tabla++;
    $titulo = (isset($apartado) ? "Tabla {$apartado}.{$num_tabla}.{$subnum_tabla}: " : '' ) . 'Perfil de ingreso de los estudiantes: género';
?>

    <div class='table-responsive'>
        <table id="tabla_<?= $centro_id ?>_genero" class="table table-striped table-hover cabecera-azul compact" style="width: 100%;">
            <caption>
