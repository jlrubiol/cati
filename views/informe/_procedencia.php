...
$subnum_tabla = 0;
foreach($procedencias as $centro_id => $datos_del_centro) {
    foreach($datos_del_centro as $tipo => $datos) {
        $centro = Centro::findOne($centro_id);
        $anyos = array_shift($datos);
        $subnum_tabla++;
        $titulo = (isset($apartado) ? "Tabla {$apartado}.{$num_tabla}.{$subnum_tabla}: " : '' ) . "Perfil de ingreso de los estudiantes: procedencia ({$tipo})";
    ?>

        <div class='table-responsive'>
            <table id="tabla_<?= $centro_id ?>_<?= $tipo ?>" class="table table-striped table-hover cabecera-azul compact" style="width: 100%;">
                <caption>
