...
// En los másteres no hay nota media de admisión
if ($estudio->esMaster()) {
...
$subnum_tabla = 0;
foreach($notasMedias as $centro_id => $datos_del_centro) {
    $centro = Centro::findOne($centro_id);
    $anyos = array_shift($datos_del_centro);
    $subnum_tabla++;
    $titulo = (isset($apartado) ? "Tabla {$apartado}.{$num_tabla}.{$subnum_tabla}: " : '' ) . 'Nota media de admisión y nota de corte';
?>

    <div class='table-responsive'>
        <table id="tabla_<?= $centro_id ?>_media" class="table table-striped table-hover cabecera-azul compact" style="width: 100%;">
            <caption>
