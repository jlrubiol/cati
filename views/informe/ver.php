<?php
/**
 * Vista de un informe de evaluación de Grado o Máster.
 *
...
foreach ($preguntas as $pregunta) {
    ...
    # Iniciamos la pregunta, poniéndola toda ella en un <div>.
    # Si es la pregunta 0 (Seguimiento del PAIM del curso anterior) o uno de sus subapartados
    # no se imprime aquí, sino que se mostrará como anexo al final.
    // if ($pregunta->apartado[0] == "0") echo '<div class="noprint">';
    // else echo "<div>";
    echo "<div>";
    ...
    if ($respuesta) {
        echo '<div>' . str_replace('<table>', "<table class='table table-bordered'>", HtmlPurifier::process(
            $respuesta->contenido,
            [
    ...
    if ($mostrar_botones && $pregunta->editable && !($estudio->anyos_evaluacion == 1 && !$pregunta->editable_1)) {
        ...
        if ($respuesta) {
            echo Html::a(
                '<span class="glyphicon glyphicon-pencil"></span> ' . Yii::t('cati', 'Editar respuesta'),
                ['informe-respuesta/editar', 'estudio_id' => $estudio->id, 'informe_respuesta_id' => $respuesta->id],
                ['id' => "editar-respuesta-{$pregunta->id}", 'class' => 'btn btn-success']
            ) . "\n";
        } else {
            echo Html::a(
                '<span class="glyphicon glyphicon-plus"></span> ' . Yii::t('cati', 'Añadir respuesta'),
                ['informe-respuesta/crear', 'estudio_id' => $estudio->id, 'informe_pregunta_id' => $pregunta->id],
    ...
    echo "</div>";  # Finalizamos la pregunta
}

####################################################################

// Al cerrar el IEC (`actionCerrar()` en `InformeController.php`),
// también se generarán PDFs de los PAIM completados (apartado 0 del IEC),
// que se anexarán al final del IEC.

# El apartado 0, que en pantalla se muestra al principio de la página,
# en el PDF se muestra al final como un anexo.
# TODO: Conseguir poner las páginas de esta sección en orientación horizontal o A3.
# Quizás generando un PDF aparte, y uniéndolo al del IEC?
/*
echo '<hr style="page-break-after: always;">';
echo '<div class="hide-from-screen a4-landscape">';

foreach ($preguntas as $pregunta) {
    if ($pregunta->apartado[0] == "0") {
...
echo '</div>';  # hide-from-screen a4-landscape
*/
...
if ($mostrar_botones) {
    ...
    /*
        echo Html::a(
            '<span class="glyphicon glyphicon-pencil"></span> ' . Yii::t('cati', 'Editar'),
            ['editar', 'estudio_id' => $estudio->id, 'anyo' => $anyo],
            [
                'id' => 'editar-informe',
                'class' => 'btn btn-success',  // Button
                'title' => Yii::t('cati', 'Seguir editando las respuestas del informe.'),
            ]
        ) . " &nbsp; \n";
    */
